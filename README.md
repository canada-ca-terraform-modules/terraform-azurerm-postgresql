...
| kv_name                                  | string           | `""`                | no       | The name to be used for the Key Vault against the PostgreSQL instance.                                                               |
| kv_rg                                    | string           | `""`                | no       | The resource group to be used for the Key Vault against the PostgreSQL instance.                                                     |
| kv_tenant_id                             | string           | `""`                | yes      | The Tenant ID to be used for the Key Vault against the PostgreSQL instance.                                                          |
...
| vnet_name                                | string           | null                | yes      | The vnet name to be used when vnet_create is either set to `true` or `false`.                                                        |
| vnet_rg                                  | string           | null                | no       | The vnet resource group to be used when vnet_create is either set to `true` or `false`.                                              |
| subnet_create                            | string           | nul;                | no       | Flag subnet_create can either be `null` (default), `true` (create vnet), or `false` (use existing vnet).                             |
| subnet_address_prefixes                  | list             | `["172.15.8.0/22"]` | no       | Virtual Network Address Prefixes.                                                                                                    |
| subnet_name                              | string           | null                | no       | The subnet name to be used when subnet_create is either set to `true` or `false`.                                                    |
